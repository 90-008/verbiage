class AppRequest {
    ...
    get searchParams() {
        ...
        let searchString = this.request.url.match(/.+\?(.+)/)
        this._searchParamsParsed = searchString != null ? new URLSearchParams(searchString[1]) : new URLSearchParams()
    ...
    getHead(name) {
        let existing = this.headers.findIndex((item) => { return item[0].toLowerCase() == name.toLowerCase() })
    ...
    reject(code = 400, message = "", data = {}) {
        this.status = code
        data.message = message

        return new ClientRejection(this, code, data)
    }
}
...
class ClientRejection {
    ...
    constructor(appreq, code = 400, data = {}) {
        if (!appreq) throw new Error("Client rejection needs an AppRequest")

        this.appreq = appreq
        this.code = code
        this.data = data

        appreq.clientRejection = this
    }
}
...
class MultipartForm {
    ...
    getMany(...names) {
        ...
        for (let name of names) { obj[name] = this.get(name) }

        return obj
    }

    forEach(name, cb) {
        this.fields.filter(field => field.name == name).forEach(field => cb(field))
    }
...
module.exports.ClientRejection = ClientRejection;
