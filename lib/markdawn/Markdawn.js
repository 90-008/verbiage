...
class Markdawn {
    ...
    renderInlineFeatures(line) {
        ...
        /* 
            TODO: This faulty regex makes it so any masked link on the same line as a former code block won't be parsed.
            Ideally this needs to be written out into a proper parser.
        */
        final = final.replace(/(?<!<code>.*)\[([^\]]+)\]\(([^\)]+)\)/g, `<a href="$2" target="_blank">$1</a>`)
