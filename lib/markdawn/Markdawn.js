...
class MarkdownLinkFeature extends InlineFeature {
    ...
    static transform = (match, dawn) => {
        ...
        if (linkParts[1]) {
            ...
            return `<a href="${linkRewritten}" ${linkRewritten.match(LINK_REGEX) ? `target="_blank"` : ``}>`
                + `${dawn.renderInlineFeatures(linkParts[0].replace("/", "&sol;"))}`
                + `</a>`
        } else {
            return match
        }
...
class FootnoteLinkFeature extends InlineFeature {
    ...
    static transform = (match, dawn) => {
        ...
        if (
            !id[0]
            || !dawn.featureDescriptions.blocks.find(f => f.feature == FootnoteFeature && f.data.id == id[1])
        ) return match

        return `<sup id="fn-ref-${id[1]}" class="footnote"><a href="#fn-${id[1]}">${id[1]}</a></sup>`
    }
}
